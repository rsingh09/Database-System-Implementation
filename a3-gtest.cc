...
TEST(TestGroupBy, SubTest7) {
	...
	try {
		...
		char *pred_s = "(s_suppkey = s_suppkey)";
		char filename1[100] = "supplier.bin";
		Schema testSchema1("catalog", "supplier");
		dbf_s.Open(filename1);
		init_lexical_parser(pred_s);
		if (yyparse() != 0) {
			cout << " Error: can't parse your CNF " << pred_s << endl;
			exit(1);
		}
		cnf_s.GrowFromParseTree(final, &testSchema1, lit_s); 
		close_lexical_parser();
		...
		cnf_ps.GrowFromParseTree(final, &testSchema, lit_ps); 
		close_lexical_parser();
		...
		cnf_p_ps.GrowFromParseTree(final, &testSchema1, &testSchema, lit_p_ps); 
		close_lexical_parser();		
		J.Use_n_Pages(8);
		int outAtts = 7 + 5;
