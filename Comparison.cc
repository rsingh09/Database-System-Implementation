...
int OrderMaker::GetNumOfAttributes()
{
	return numAtts;
}

void OrderMaker::GetAttributes(int * attList, Type * attType)
{
	for (int i = 0; i < numAtts; i++)
	{
		attList[i] = this->whichAtts[i];
		attType[i] = this->whichTypes[i];
	}
}
void OrderMaker::GetAttributes(int * attList)
{
	for (int i = 0; i < numAtts; i++)
	{
		attList[i] = this->whichAtts[i];
...
void OrderMaker::SaveAttributes(int numofAtts, int * attList, Type * attType)
{
	this->numAtts = numofAtts;
	for (int i = 0; i < numofAtts; i++)
	{
		this->whichAtts[i] = attList[i];
		this->whichTypes[i] = attType[i];
	}
}

int OrderMaker::prepareQueryOrder(OrderMaker& sortOrder, CNF& cnf, OrderMaker& queryOrder, OrderMaker& cnfOrder)
{
	queryOrder.numAtts = 0;
	cnfOrder.numAtts = 0;

	//Iterating through the sortOrder attributes
	for (int i = 0; i < sortOrder.numAtts; i++)
	{
		bool found = false;
		//Look for the ith attribute of the sortOrder in any of the disjuntions of the CNF
		for (int j = 0; j < cnf.numAnds; j++)
		{
			Comparison currComp[MAX_ORS] = cnf.orList[j];
			int currLen = cnf.orLens[j];

			//If there is only a single disjuntion in the AND clause
			if (currLen == 1)
			{
				Comparison& cmp = currComp[0];
				//If the operations is equals and the attributes match with the operands of the Comparison
				if (cmp.op == Equals
					&& (((cmp.whichAtt1 == sortOrder.whichAtts[i]) && (cmp.operand2 == Literal))
						|| (cmp.whichAtt2 == sortOrder.whichAtts[i]) && (cmp.operand1 == Literal)))
				{
					queryOrder.whichAtts[queryOrder.numAtts] = cmp.whichAtt1;
					cnfOrder.whichAtts[cnfOrder.numAtts] = cmp.whichAtt2;
					//Assing the respective attributes based on the Literal position
					if(cmp.operand1 == Literal)
					{
						cnfOrder.whichAtts[cnfOrder.numAtts] = cmp.whichAtt1;
						queryOrder.whichAtts[queryOrder.numAtts] = cmp.whichAtt2;
					}
					...
					//We found some attribute
					found = true;
				}
			}
		}
		//If there is not matching attribute, break the loop
		if (!found)
			return queryOrder.numAtts;
	}
	//Returning the number of the records
	return queryOrder.numAtts;
}
